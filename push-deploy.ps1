...
$sshCmd = "cd /var/www/xurshid && git pull origin main && source venv/bin/activate && pip install -r requirements.txt -q && sudo cp /var/www/xurshid/nginx_sergeli0606.conf /etc/nginx/sites-enabled/xurshid && sudo nginx -t && sudo nginx -s reload && sudo systemctl restart xurshid && sleep 2 && sudo systemctl status xurshid --no-pager -n 5"
